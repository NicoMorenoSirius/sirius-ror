class WeatherInteraction < ActiveInteraction::Base
  ...
  def execute
    ...
    return WeatherDecorator.new(JSON.parse(cached_weather), true).to_json if cached_weather.present?

    WeatherDecorator.new(fetch_weather(inputs.slice(:zip_code, :appid)), false).to_json
  end
